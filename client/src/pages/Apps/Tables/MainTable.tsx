...
const MainTable = () => {
    ...
    const [urunlist,setUrunList]=useState<any>([])
    const [urunid,setUrunId]=useState<any>("10001112007")
    ...
    useEffect(()=>{
        ...
        url: `http://localhost:3300/sap/urunler/${urunid}`,
        headers: { }
        };

        axios.request(config)
        .then((response) => {
        setUrunList(response.data);
        }).finally(()=>{
        })
        .catch((error) => {
        console.log(error);
    ...
    const [initialRecords, setInitialRecords] = useState(sortBy(urunlist.list, 'LGORT'));
    const [recordsData, setRecordsData] = useState(initialRecords);

    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'LGORT', direction: 'asc' });
        ...
        <div>
            ...
            <div className="panel mt-6">
                ...
                <div className="datatables">
                    {urunlist&&urunlist.code==="success"?<DataTable
                        noRecordsText="No results match your search query"
                        highlightOnHover
                        className="whitespace-nowrap table-hover"
                        records={urunlist?.list}
                        columns={[
                            { accessor: 'MATNR', title: 'ID', sortable: true, render: ({ MATNR }:any) => <strong className="text-info">#{MATNR}</strong> },
                            {
                                accessor: 'MAKTX',
                                title: 'Malzeme Adı',
                                sortable: true,
                                render: ({ MAKTX }) => (
                                    <div className="flex items-center gap-2">
                                        <div className="font-semibold">{MAKTX}</div>
                                    </div>
                                ),
                            },
                            {
                            ...
                            { accessor: 'STPRS', title: 'Fiyat', sortable: true },
